...
# other files we need
require 'muren/indifferent_hash'
require 'muren/show_exceptions'
require 'muren/version'
...
module Muren
  ...
  class Request < Rack::Request
    ...
    def accept
      @env['muren.accept'] ||= if @env.include?('HTTP_ACCEPT') && (@env['HTTP_ACCEPT'].to_s != '')
                                 @env['HTTP_ACCEPT']
                                   .to_s
  ...
  module Helpers
    ...
    def body(value = nil, &block)
      if block_given?
        ...
        # Re-compute content-length only for simple array bodies, not for
        # streaming or HEAD responses where it is unused or incorrect.
        if !request.head?
          body = response.body
          if body.is_a?(Array)
  ...
  # Base class for all Muren applications and middleware.
  class Base
    include Rack::Utils
    include Helpers
    include Templates
    ...
    def call!(env) # :nodoc:
      ...
      invoke { dispatch! }
      invoke { error_block!(response.status) } unless @env['muren.error']
    ...
    # If there are no more matching routes, Muren will
    ...
    def route!(base = settings, pass_block = nil)
      ...
      routes&.each do |pattern, conditions, block|
        ...
        returned_pass_block = process_route(pattern, conditions) do |*args|
          env['muren.route'] = "#{@request.request_method} #{pattern}"
    ...
    def process_route(pattern, conditions, block = nil, values = [])
      ...
      @env['muren.error.params'] = @params
      raise
    ensure
      params ||= {}
      params.each_key { |k| @params.delete(k) } unless @env['muren.error.params']
    end
    ...
    def dispatch!
      ...
      begin
        filter! :after unless env['muren.static_file']
      rescue ::Exception => e
        invoke { handle_exception!(e) } unless @env['muren.error']
      end
    end

    # Error handling during requests.
    def handle_exception!(boom)
      error_params = @env['muren.error.params']

      @params = @params.merge(error_params) if error_params

      @env['muren.error'] = boom

      http_status = if boom.is_a? Muren::Error
    ...
    class << self
      CALLERS_TO_IGNORE = [ # :nodoc:
        %r{/muren(/(base|main|show_exceptions))?\.rb$},   # all muren code
      ...
      def error(*codes, &block)
        ...
        codes << Muren::NotFound if codes.include?(404)
      ...
      # Set configuration options for Muren and/or the app.
      ...
      def quit!
        ...
        warn '== Muren has ended his set (crowd applauds)' unless suppress_messages?
      ...
      # Run the Muren app as a self-hosted server using
      ...
      def run!(options = {}, &block)
        unless defined?(Rackup::Handler)
          rackup_warning = <<~MISSING_RACKUP
            Muren could not start, the required gems weren't found!
      ...
      def start_server(handler, server_settings, handler_name)
        ...
        handler.run(self, **server_settings) do |server|
          unless suppress_messages?
            warn "== Muren (v#{Muren::VERSION}) has taken the stage on #{port} for #{environment} with backup from #{handler_name}"
          end
      ...
      def setup_null_logger(builder)
        builder.use Muren::Middleware::Logger, ::Logger::FATAL
      end

      def setup_common_logger(builder)
        builder.use Muren::CommonLogger
      end

      def setup_custom_logger(builder)
        if logging.respond_to? :to_int
          builder.use Muren::Middleware::Logger, logging
        else
          builder.use Muren::Middleware::Logger
        end
      end
    ...
    configure :development do
      ...
      error NotFound do
        ...
        if instance_of?(Muren::Application)
          code = <<-RUBY.gsub(/^ {12}/, '')
        ...
        <<-HTML.gsub(/^ {10}/, '')
          ...
          <body>
            <h2>Muren doesn’t know this ditty.</h2>
            <img src='#{request.script_name}/__muren__/404.png'>
            <div id="c">
              Try this:
  ...
  # top-level. Subclassing Muren::Base is highly recommended for
  ...
  # Muren delegation mixin. Mixing this module into an object causes all
  # methods to be delegated to the Muren::Application class. Used primarily
  # at the top-level.
  ...
  # Create a new Muren application; the block is evaluated in the class scope.
  ...
  # Include the helper modules provided in Muren's request context.
